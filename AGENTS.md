...
2. **Platform toggles** — cross-platform extensions are *never* hidden; the toggles only affect platform-exclusive ones. `removed` items are exempt, because their `platforms` value is a placeholder rather than real data. `useFilterToggles` guarantees at least one platform stays enabled.
...
Without the optional `githubToken` preference the budget is 60 req/hr per IP, shared across both commands. `fetchMergedPRs()` in `src/utils/index.ts` is the single entry point and picks exactly **one** transport: GraphQL when the user opted in *and* supplied a token, REST otherwise and on any GraphQL failure.

> The view command uses `useCachedPromise`, **not** `useFetch`. `useFetch` issues the request itself and only then calls `parseResponse`, so a GraphQL branch there would run *after* a REST call had already been spent — paying for both. Whatever owns the transport choice must also own the request.

Two cost controls, both load-bearing:

- Slug guesses come from fields already in the PR list response (labels, title, head branch), so most PRs resolve with no extra billed request. When the first guess misses, only an `extension:` label is trusted to *replace* it — a branch name matching another extension would otherwise produce the wrong Store URL.
